...
public class CallTenderOfferImportServiceImpl implements CallTenderOfferImportService {
  ...
  protected void applyCustomFieldValues(
    ...
    Map<String, MetaJsonField> fieldsByTitle = getCustomFieldsByTitle(product);
      ...
      for (int colIdx = COL_CUSTOM_FIELDS_START; colIdx < commentColIdx; colIdx++) {
        ...
        if (field == null) {
          if (!rawValue.isEmpty()) {
            rowErrors.add(
                String.format(
                    I18n.get("Line %d: Field '%s' is not configured for product '%s'"),
                    lineNumber,
                    header,
                    product.getCode()));
            appendImportError(
                offer,
                "Field '" + header + "' is not configured for product '" + product.getCode() + "'");
          }
          continue;
        }
  ...
  protected Map<String, MetaJsonField> getCustomFieldsByTitle(Product product) {
    ...
    for (MetaJsonField f : config.getCustomFieldList()) {
      if (!CallTenderAttrConfigService.MODEL_NEED.equals(f.getModel())) {
        continue;
      }
